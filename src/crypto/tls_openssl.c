...
/* OpenSSL 3.0 made many APIs deprecated; suppress warnings for compatibility code */
				  ...
				  unsigned char *to, RSA *rsa, int padding)
{
	struct cryptoapi_rsa_data *priv =
		(struct cryptoapi_rsa_data *) RSA_meth_get0_app_data(RSA_get_method(rsa));
	HCRYPTHASH hash;
...
static int cryptoapi_finish(RSA *rsa)
{
	...
	cryptoapi_free_data((struct cryptoapi_rsa_data *) RSA_meth_get0_app_data(meth));
...
static int tls_cryptoapi_cert(SSL *ssl, const char *name)
{
	...
	rsa_meth = RSA_meth_new("Microsoft CryptoAPI RSA Method", 0);
	if (rsa_meth == NULL) {
		wpa_printf(MSG_WARNING, "CryptoAPI: Failed to allocate memory "
			   "for CryptoAPI RSA method");
		os_free(priv);
	...
	RSA_meth_set_flags(rsa_meth, RSA_METHOD_FLAG_NO_CHECK);
